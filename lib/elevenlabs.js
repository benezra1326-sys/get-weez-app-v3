...
class ElevenLabsVoice {
  constructor(apiKey) {
    ...
    // Voix disponibles - Voix françaises avec accent français
    this.voices = {
      victoire: 'O31r762Gb3WFygrEOGh0', // Victoire (voix féminine française)
      sebas: '5jCmrHdxbpU36l1wb3Ke', // Sébas (voix masculine française)
      rachel: '21m00Tcm4TlvDq8ikWAM', // Rachel (voix féminine anglaise - désactivée)
      domi: 'AZnzlk1XvdvUeBnXmlld' // Domi (voix féminine anglaise - désactivée)
    }
  ...
  getVoiceId() {
    if (typeof window !== 'undefined') {
      ...
      // Forcer les voix françaises seulement
      if (preference === 'victoire' || preference === 'sebas') {
        return this.voices[preference]
      }
      // Par défaut, utiliser Victoire (voix française)
      return this.voices.victoire
    }
    
    return this.voices.victoire // Par défaut - voix française
  }
  ...
  async textToSpeech(text, options = {}) {
    ...
    try {
      const response = await fetch(`${this.baseUrl}/text-to-speech/${this.voiceId}`, {
        ...
        body: JSON.stringify({
          text,
          model_id: options.modelId || 'eleven_multilingual_v2', // Modèle multilingue pour le français
          voice_settings: {
            stability: options.stability || 0.6, // Plus stable pour le français
            similarity_boost: options.similarityBoost || 0.8, // Plus similaire à la voix originale
            style: options.style || 0.3, // Moins de style pour un français plus naturel
