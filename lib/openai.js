...
## 📅 GESTION DES DATES ET ÉVÉNEMENTS
- **IMPORTANT** : Toujours filtrer les événements selon la date demandée
- Si l'utilisateur demande "ce soir", "aujourd'hui" → proposer uniquement les événements du jour même
- Si l'utilisateur demande "demain" → proposer uniquement les événements du lendemain
- Si l'utilisateur demande "ce weekend" → proposer les événements du weekend en cours
- **NE JAMAIS** proposer des événements passés ou trop éloignés dans le temps
- Pour "ce soir" (10 octobre), ne proposer QUE les événements du 10 octobre, pas du 14, 15 ou 24 octobre
- Si aucun événement n'est disponible pour la date demandée, proposer des alternatives (restaurants, bars, clubs) qui sont ouverts
...
export function filterEventsByDate(message, allEvents) {
  const msg = message.toLowerCase()
  const today = new Date()
  const todayStr = today.toISOString().split('T')[0] // Format YYYY-MM-DD
  
  // Détecter la demande de date
  let targetDate = null
  
  if (msg.includes('ce soir') || msg.includes('aujourd\'hui') || msg.includes('tonight') || msg.includes('today')) {
    targetDate = todayStr
  } else if (msg.includes('demain') || msg.includes('tomorrow')) {
    const tomorrow = new Date(today)
    tomorrow.setDate(today.getDate() + 1)
    targetDate = tomorrow.toISOString().split('T')[0]
  } else if (msg.includes('ce weekend') || msg.includes('this weekend')) {
    ...
    return allEvents.filter(event => {
      ...
      return eventDate === saturday.toISOString().split('T')[0] || 
             eventDate === sunday.toISOString().split('T')[0]
    })
  }
  ...
  // Si pas de filtre de date spécifique, retourner tous les événements futurs
  return allEvents.filter(event => event.date >= todayStr)
}

// Fonction pour ajouter des événements du jour si aucun événement spécifique
export function getAlternativeOptionsForToday() {
  return {
    establishments: establishments.filter(est => 
      est.type === 'Restaurant' || est.type === 'Bar' || est.type === 'Club' || est.type === 'Nightclub'
    ).slice(0, 5), // Limiter à 5 suggestions
    message: "Aucun événement spécial ce soir, mais voici quelques excellents endroits pour sortir :"
  }
}
...
export async function askWeezAgent(message, userName = 'Utilisateur', isMember = false, conversationHistory = []) {
  ...
  try {
    ...
    // Filtrer les événements par date si nécessaire
    const filteredEvents = filterEventsByDate(message, events)
    console.log(`📅 Événements filtrés pour "${message}": ${filteredEvents.length} trouvés`)

    // Utiliser le prompt système adapté à la langue
    const systemPrompt = languageDetector.getSystemPrompt(detectedLanguage)

    // Ajouter les données filtrées au contexte
    const eventsContext = filteredEvents.length > 0 
      ? `\n\nÉVÉNEMENTS DISPONIBLES POUR CETTE DATE :\n${filteredEvents.map(event => `- ${event.name} (${event.date} à ${event.time}) - ${event.location}`).join('\n')}`
      : `\n\nAUCUN ÉVÉNEMENT SPÉCIAL POUR CETTE DATE. Proposer des alternatives (restaurants, bars, clubs ouverts).`
...
Réponds de manière friendly, spécifique et incite à la réservation.`
    ...
    // Fallback intelligent basé sur le nouveau prompt avec filtrage par date
...
function generateGliitzFallbackResponse(message, language = 'fr') {
  ...
  // Filtrer les événements par date pour le fallback aussi
  const filteredEvents = filterEventsByDate(message, events)
  console.log(`📅 Fallback - Événements filtrés pour "${message}": ${filteredEvents.length} trouvés`)
  ...
  // Gestion spéciale pour les demandes de sortie ce soir
  if (msg.includes('ce soir') || msg.includes('sortir') || msg.includes('tonight')) {
    if (filteredEvents.length > 0) {
      const eventsList = filteredEvents.slice(0, 3).map(event => 
        `• **${event.name}** - ${event.location} (${event.time})`
      ).join('\n')
      
      return language === 'fr'
        ? `🎉 Parfait ! Voici les événements disponibles ce soir à Marbella :\n\n${eventsList}\n\nQuel événement vous intéresse ? Je peux vous aider à réserver ! ✨`
        : `🎉 Perfect! Here are the events available tonight in Marbella:\n\n${eventsList}\n\nWhich event interests you? I can help you book! ✨`
    } else {
      // Aucun événement ce soir, proposer des alternatives
      const alternatives = getAlternativeOptionsForToday()
      const alternativesList = alternatives.establishments.slice(0, 3).map(est => 
        `• **${est.name}** - ${est.type} (${est.location || 'Marbella'})`
      ).join('\n')
      
      return language === 'fr'
        ? `🎭 ${alternatives.message}\n\n${alternativesList}\n\nQuel endroit vous tente ? Je peux vous aider à réserver ! ✨`
        : `🎭 No special events tonight, but here are some great places to go out:\n\n${alternativesList}\n\nWhich place interests you? I can help you book! ✨`
    }
  }
