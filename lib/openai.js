...
// Fonction pour générer des recommandations basées sur la base de données
async function generateRecommendationFromDatabase(message, establishments, events, activities) {
  const messageLower = message.toLowerCase()
  
  // Recherche d'établissements spécifiques
  const specificEstablishment = findSpecificEstablishment(messageLower, establishments)
  if (specificEstablishment) {
    return generateEstablishmentRecommendation(specificEstablishment, messageLower)
  }
  
  // Recherche par services et plats spécifiques
  const serviceRecommendations = findEstablishmentsByServices(messageLower, establishments)
  if (serviceRecommendations.length > 0) {
    return generateServiceBasedRecommendations(serviceRecommendations, messageLower)
  }
  
  // Recherche par plats spécifiques
  const dishRecommendations = findEstablishmentsByDishes(messageLower, establishments)
  if (dishRecommendations.length > 0) {
    return generateDishBasedRecommendations(dishRecommendations, messageLower)
  }
  
  // Recherche par catégorie générale
  if (messageLower.includes('plage') || messageLower.includes('beach')) {
    return generateBeachRecommendations(establishments, messageLower)
  }
  
  if (messageLower.includes('restaurant') || messageLower.includes('diner') || messageLower.includes('manger')) {
    return generateRestaurantRecommendations(establishments, messageLower)
  }
  
  if (messageLower.includes('club') || messageLower.includes('sortir') || messageLower.includes('soir')) {
    return generateClubRecommendations(establishments, messageLower)
  }
  
  if (messageLower.includes('villa') || messageLower.includes('evg')) {
    return generateVillaRecommendations(establishments, messageLower)
  }
  
  return null
}

// Trouver des établissements par services spécifiques
function findEstablishmentsByServices(message, establishments) {
  const serviceKeywords = {
    'spa': ['spa', 'massage', 'relaxation', 'détente', 'soins', 'wellness'],
    'piscine': ['piscine', 'pool', 'swimming', 'bain', 'aqua'],
    'terrasse': ['terrasse', 'terrace', 'extérieur', 'dehors', 'balcon'],
    'vue mer': ['vue mer', 'sea view', 'océan', 'vue sur la mer', 'panoramique', 'mer'],
    'parking': ['parking', 'garage', 'voiture', 'stationnement'],
    'wifi': ['wifi', 'internet', 'connexion', 'web'],
    'climatisation': ['climatisation', 'air conditionné', 'ac', 'climatisé'],
    'dj': ['dj', 'musique', 'mix', 'son', 'concert', 'live'],
    'bar': ['bar', 'cocktail', 'boisson', 'boire', 'alcool'],
    'cuisine': ['cuisine', 'kitchen', 'repas', 'manger', 'restaurant']
  }
  ...
  return [...new Set(matchingEstablishments)] // Supprimer les doublons
}

// Trouver des établissements par plats spécifiques
...
// Générer des recommandations basées sur les services
function generateServiceBasedRecommendations(establishments, message) {
  if (establishments.length === 0) return null
  
  let recommendation = "Parfait ! Je vois que tu cherches des services spécifiques. Voici mes recommandations :\n\n"
  
  establishments.slice(0, 3).forEach((establishment, index) => {
    const { name, type, category, rating, features, sponsored, price_range } = establishment
    
    recommendation += `⭐ **${name}**\n`
    recommendation += `   • ${type} ${category.toLowerCase()} - Note : ${rating}/5\n`
    if (price_range) recommendation += `   • Prix : ${price_range}\n`
    if (features) recommendation += `   • Services : ${features.slice(0, 4).join(', ').toLowerCase()}\n`
    if (sponsored) recommendation += `   • Partenaire privilégié Get Weez !\n`
    recommendation += `\n`
  })
  
  recommendation += "Je peux réserver ces services pour toi ! Dis-moi ce qui t'intéresse le plus."
  
  return recommendation
}

// Générer des recommandations basées sur les plats
function generateDishBasedRecommendations(establishments, message) {
  if (establishments.length === 0) return null
  
  let recommendation = "Excellent ! Je vois que tu cherches des plats spécifiques. Voici mes recommandations :\n\n"
  
  establishments.slice(0, 3).forEach((establishment, index) => {
    const { name, type, category, rating, specialties, dishes, sponsored, price_range } = establishment
    
    recommendation += `🍽️ **${name}**\n`
    recommendation += `   • ${type} ${category.toLowerCase()} - Note : ${rating}/5\n`
    if (price_range) recommendation += `   • Prix : ${price_range}\n`
    if (specialties) recommendation += `   • Spécialités : ${specialties.slice(0, 3).join(', ')}\n`
    if (dishes) recommendation += `   • Plats : ${dishes.slice(0, 2).join(', ')}\n`
    if (sponsored) recommendation += `   • Partenaire privilégié Get Weez !\n`
    recommendation += `\n`
  })
  
  recommendation += "Je peux réserver une table et commander ces plats pour toi !"
  
  return recommendation
}

// Trouver un établissement spécifique
function findSpecificEstablishment(message, establishments) {
  for (const establishment of establishments) {
    const nameLower = establishment.name.toLowerCase()
    // Recherche exacte ou partielle du nom
    if (message.includes(nameLower) || nameLower.includes(message)) {
      return establishment
    }
    // Recherche par mots clés du nom (ex: "nobu" pour "Nobu Marbella")
    const nameWords = nameLower.split(' ')
    if (nameWords.some(word => message.includes(word) && word.length > 3)) {
      return establishment
    }
  }
...
// Générer une recommandation pour un établissement spécifique
function generateEstablishmentRecommendation(establishment, message) {
  const { name, type, category, description, rating, price_range, ambiance, features, sponsored, zone } = establishment
  ...
  recommendation += `Je peux réserver pour toi !`
  
  return recommendation
}

// Générer des recommandations de plages
function generateBeachRecommendations(establishments, message) {
  const beaches = establishments.filter(e => e.type === 'Plage' || e.name.toLowerCase().includes('beach') || e.name.toLowerCase().includes('plage'))
  
  if (beaches.length === 0) return null
  
  let recommendation = "Parfait ! Pour une plage à Marbella, voici mes recommandations :\n\n"
  
  beaches.forEach((beach, index) => {
    const { name, category, rating, price_range, ambiance, features, sponsored } = beach
    
    recommendation += `🏖️ **${name}**\n`
    recommendation += `   • ${category} - Note : ${rating}/5\n`
    if (price_range) recommendation += `   • Transats à partir de 25€\n`
    if (ambiance) recommendation += `   • Ambiance : ${ambiance.toLowerCase()}\n`
    if (features) recommendation += `   • ${features.join(', ').toLowerCase()}\n`
    if (sponsored) recommendation += `   • Partenaire privilégié Get Weez !\n`
    recommendation += `\n`
  })
  
  recommendation += "Je peux réserver des transats VIP pour toi !"
  
  return recommendation
}

// Générer des recommandations de restaurants
function generateRestaurantRecommendations(establishments, message) {
  const restaurants = establishments.filter(e => e.type === 'Restaurant')
  
  if (restaurants.length === 0) return null
  
  let recommendation = "Excellent ! Pour un restaurant à Marbella, voici mes recommandations :\n\n"
  
  restaurants.slice(0, 3).forEach((restaurant, index) => {
    const { name, category, rating, price_range, ambiance, specialties, sponsored } = restaurant
    
    recommendation += `🍽️ **${name}**\n`
    recommendation += `   • Cuisine ${category.toLowerCase()} - Note : ${rating}/5\n`
    if (price_range) recommendation += `   • Prix : ${price_range}\n`
    if (ambiance) recommendation += `   • Ambiance : ${ambiance.toLowerCase()}\n`
    if (specialties) recommendation += `   • Spécialités : ${specialties.slice(0, 3).join(', ')}\n`
    if (sponsored) recommendation += `   • Partenaire privilégié Get Weez !\n`
  ...
  recommendation += "Je peux réserver une table VIP pour toi !"
  
  return recommendation
}

// Générer des recommandations de clubs
function generateClubRecommendations(establishments, message) {
  const clubs = establishments.filter(e => e.type === 'Club')
  
  if (clubs.length === 0) return null
  
  let recommendation = "Super ! Pour sortir à Marbella, voici mes recommandations :\n\n"
  
  clubs.slice(0, 3).forEach((club, index) => {
    const { name, category, rating, ambiance, features, sponsored } = club
  ...
  recommendation += "Je peux réserver des tables VIP pour toi !"
  
  return recommendation
}

// Générer des recommandations de villas
function generateVillaRecommendations(establishments, message) {
  const villas = establishments.filter(e => e.type === 'Villa')
  
  if (villas.length === 0) return null
  
  let recommendation = "Parfait ! Pour une villa à Marbella, voici mes recommandations :\n\n"
  
  villas.slice(0, 3).forEach((villa, index) => {
    const { name, category, rating, capacity, features, sponsored } = villa
    
    recommendation += `🏡 **${name}**\n`
    recommendation += `   • ${category} - Note : ${rating}/5\n`
    if (capacity) recommendation += `   • Capacité : ${capacity} personnes\n`
    if (features) recommendation += `   • Équipements : ${features.join(', ').toLowerCase()}\n`
    if (sponsored) recommendation += `   • Partenaire privilégié Get Weez !\n`
    recommendation += `\n`
  })
  
  recommendation += "Je peux organiser la réservation pour toi !"
...
export async function askWeezAgent(message, userName, isMember, conversationHistory = []) {
  try {
    ...
    // Désactiver l'ancien système qui donne des réponses génériques
    ...
    let systemPrompt = `Tu es "Get Weez", un concierge de luxe EXPERT à Marbella. Tu es FRIENDLY, BIENVEILLANT, DÉVOUÉ au client et tu DONNES TOUJOURS des réponses concrètes et personnalisées.
...
12. ADAPTE tes recommandations selon le profil client
13. ANTICIPE les besoins futurs de l'utilisateur
14. OBJECTIF FINAL : TOUJOURS proposer de RÉSERVER
15. PROPOSE des établissements par SERVICES et PLATS spécifiques
16. VEND les partenaires privilégiés avec leurs qualités
17. ORGANISE tout : réservations, services, expériences
18. Sois FRIENDLY et BIENVEILLANT - tu es là pour aider le client
19. RECONNAIS quand tu ne comprends pas et DEMANDE des précisions avec bienveillance
20. CORRIGE tes erreurs immédiatement et excuse-toi si nécessaire
21. SOIS DÉVOUÉ au client - son bonheur est ta priorité
22. INCITE toujours à la RÉSERVATION de manière naturelle et bienveillante
...
Message : "bonjour"
Réponse : "Salut ! 😊 Je suis Get Weez, ton concierge personnel dévoué à Marbella ! Je suis là pour te faire vivre des expériences exceptionnelles. Je peux t'aider avec des villas de luxe, restaurants exclusifs, clubs VIP, plages privées, yachts, jets privés, et bien plus ! Que puis-je organiser pour toi aujourd'hui ?"

Message : "je ne sais pas ce que je veux"
Réponse : "Pas de problème ! 😊 Je suis là pour t'aider à découvrir Marbella. Laisse-moi te proposer quelques expériences selon ton humeur : pour te détendre, j'ai des spas de luxe et des plages tranquilles. Pour une soirée festive, des clubs exclusifs avec DJ internationaux. Pour un moment romantique, des restaurants avec vue mer. Ou si tu veux de l'aventure, des yachts privés ! Qu'est-ce qui te tente le plus ? Je peux tout organiser !"

Message : "un restaurant ce soir"
Réponse : "Parfait ! Pour ce soir à Marbella, je te recommande **La Terraza del Mar** - vue imprenable sur la mer et cuisine méditerranéenne raffinée. Ou **Nobu Marbella** pour une expérience japonaise de luxe. Je peux réserver une table VIP pour ce soir !"

Message : "je veux du sushi"
Réponse : "Excellent ! Pour du sushi à Marbella, **Nobu Marbella** est parfait - restaurant japonais de luxe avec sushi frais et omakase exclusif. Ou **Katsuya** pour une expérience japonaise authentique. Je peux réserver et commander pour toi !"

Message : "un endroit avec spa"
Réponse : "Parfait ! Pour un spa à Marbella, je te recommande **Villa Marbella Club** avec spa privé et massages de luxe, ou **Puente Romano Beach** avec spa en bord de mer. Je peux réserver tes soins VIP !"

Message : "un endroit avec piscine"
Réponse : "Parfait ! Pour un endroit avec piscine à Marbella, je te recommande **Nikki Beach** avec sa piscine exclusive et vue sur la mer, ou **Villa Marbella Club** avec piscine privée et spa. Je peux réserver pour toi !"
...
ADAPTE ta réponse selon le profil client et le contexte analysé.`
    ...
    // En cas d'erreur, utiliser la base de données pour des recommandations précises
    console.log('🔄 Génération d\'une réponse basée sur la base de données...')
    
    // Analyser le message pour donner une réponse basée sur la base de données
    const messageLower = message.toLowerCase()
    const recommendation = await generateRecommendationFromDatabase(messageLower, establishments, events, activities)
    
    if (recommendation) {
      return recommendation
    }
    
    if (messageLower.includes('plage') || messageLower.includes('beach') || messageLower.includes('casanis') || messageLower.includes('trocadero')) {
      ...
      if (messageLower.includes('trocadero')) {
        return "Excellent choix ! **Trocadero** est parfait pour une ambiance plus calme - plage tranquille avec transats confortables et service discret. C'est l'endroit idéal pour se détendre loin de l'agitation. Je peux réserver des transats VIP pour toi à Trocadero ! Si tu veux encore plus de calme, je peux aussi te proposer **Playa de la Fontanilla**."
      }
      if (messageLower.includes('calme') || messageLower.includes('tranquille')) {
        return "Parfait ! Pour une ambiance plus calme, je te recommande **Trocadero** - plage tranquille avec transats confortables et service discret. Ou **Playa de la Fontanilla** pour encore plus de sérénité. Ces plages sont parfaites pour se détendre loin de l'agitation !"
      }
