...
Réponds de manière friendly, spécifique et incite à la réservation.`
    ...
    try {
      // Utiliser le système de fallback amélioré avec détection de langue
      const analysis = improvedFallback.analyzeMessage(message)
      const fallbackResponse = improvedFallback.generateResponse(analysis)
      
      // Adapter la réponse à la langue détectée
      const localizedResponse = adaptResponseToLanguage(fallbackResponse, detectedLanguage)
      ...
      // Dernière ligne de défense avec langue appropriée
      const errorResponse = languageDetector.getRandomResponse('error', detectedLanguage)
