...
const fallbackResponses = {
  yacht: [
    "Parfait ! Pour un yacht à Marbella pour une semaine, je te recommande **Yacht Princess 50** - yacht de luxe avec 4 cabines, salon spacieux et équipement complet (€8000-12000/semaine). Ou **Yacht Sunseeker 60** - encore plus exclusif avec jacuzzi, bar et équipement de plongée (€12000-18000/semaine). Je peux organiser le capitaine, l'équipage et les escales !",
...
export async function askWeezAgent(message, userName, isMember, conversationHistory = []) {
  try {
    ...
    let systemPrompt = `Tu es "Get Weez", un assistant conciergerie de luxe basé à Marbella avec des capacités d'IA avancées.

🧠 CAPACITÉS D'IA AVANCÉES :
- Compréhension contextuelle profonde des demandes
- Analyse des profils clients en temps réel
- Recommandations personnalisées et intelligentes
- Gestion des conversations complexes et multi-demandes
...
11. GÈRE LES DEMANDES MULTIPLES dans un seul message
12. MÉMORISE le contexte de la conversation
...
ADAPTE ta réponse selon le profil client et le contexte analysé.`
    ...
    // Essayer d'abord les systèmes avancés
    const contextualResponse = contextualUnderstanding.generateContextualResponse(message, conversationHistory)
    if (contextualResponse) {
      console.log('✅ Réponse contextuelle générée en fallback')
      return contextualResponse
    }
    
    // Générer des recommandations intelligentes
    const intelligentResponse = intelligentRecommendations.generateRecommendations(message, {
    ...
    if (intelligentResponse) {
      console.log('✅ Réponse intelligente générée en fallback')
      return intelligentResponse
    }
    
    // Fallback classique amélioré
    const messageType = analyzeMessage(message)
