...
import { MessageCircle, Sun, Moon, Sparkles, X, Trash2, Mic, MicOff, Send } from 'lucide-react'
 ...
 * Contient la zone de messages, l'input et la toolbar mobiles 
 */
const ChatArea = memo(({ 
  messages,
  ...
  const renderConversationHeader = useCallback(() => {
    ...
    return (
      <div className={`flex items-center justify-between p-4 border-b ${themeClasses.surface} backdrop-blur-sm`}>
  ...
  const renderInputArea = useCallback(() => {
    ...
    return (
      <div className={styles.inputArea}>
        ...
        {/* Instructions */}
        <div className={`flex items-center justify-between text-xs ${themeClasses.text.muted} mt-2`}>
  ...
  return (
    <div className={`${styles.chatArea} ${className}`}>
      <div className={`${styles.chatContainer} ${themeClasses.main}`}>
        ...
        {/* Messages container */}
        <div className={`${styles.messagesContainer} scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-gray-800`} style={{ paddingTop: '1.5rem' }}>
          {renderMessages()}
        </div>
